...
def lambda_handler(event, context):
    ...
    if 'City' in intent['slots'] and 'value' in intent['slots']['City']:
        ...
        # Grab the latitude and longitude from the Google Maps API to pass to the eBird API
        # If there is more than one city with the same name in different states, check to see if the user specified a state
        # Otherwise, default to the first entry
        if 'State' in intent['slots'] and len(intent['slots']['State']) == 2:
            for location in locationData['results']:
                stateIndex = getStateNameIndex(location)
            ...
            stateIndex = getStateNameIndex(locationData['results'][0])
            state = locationData['results'][0]['address_components'][stateIndex]['long_name']
    ...
    # Problem with the city and/or state provided        

            
    else:
        print('went into else case')
        should_end_session = False
